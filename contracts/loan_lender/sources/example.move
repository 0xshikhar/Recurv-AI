...
module loan_lender::example {
    ...
    public fun new<T>(
        ...
        let loan_lender_id = object::uid_to_inner(&id);
        let loan_lender = FlashLender { id, to_lend, fee };

        // make the `FlashLender` a shared object so anyone can request loans
        transfer::share_object(loan_lender);

        // give the creator admin permissions
        AdminCap { id: object::new(ctx), loan_lender_id }
    }
    ...
    public fun loan<T>(
        ...
        let loan_lender_id = object::id(self);
        let receipt = Receipt { loan_lender_id, repay_amount };
    ...
    public fun repay<T>(
        ...
        let Receipt { loan_lender_id, repay_amount } = receipt;

        assert!(object::id(self) == loan_lender_id, ERepayToWrongLender);
    ...
    public fun loan_lender_id<T>(self: &Receipt<T>): ID {
        self.loan_lender_id
    }
    ...
    public fun withdraw<T>(
        ...
        assert!(object::borrow_id(self) == &admin.loan_lender_id, EAdminOnly);
