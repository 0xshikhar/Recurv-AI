...
contract Swapper is Ownable, ReentrancyGuard {
    ...
    constructor() Ownable(msg.sender) {
        ...
        tokens[0] = Token({
            tokenpricefeed: AggregatorV3Interface(
                0x91f3Ff344623aDC499eC6A34fC6311e8Abbf7880
            ),
        ...
        tokens[1] = Token({
            tokenpricefeed: AggregatorV3Interface(
                0xAE17aC6B7565176B9dDAD32E0dFFdC52A221b351
            ),
        ...
        tokens[2] = Token({
            tokenpricefeed: AggregatorV3Interface(
                0x7a0335B768C855792F225626F18de5291f142Ec9
            ),
        ...
        tokens[3] = Token({
            tokenpricefeed: AggregatorV3Interface(
                0xB5d3e4080dF612d33E78A523c9F4d3362ee2EC48
            ),
    ...
    function swapTokensForTokens(
        ...
        require(
            _fromTokenId < tokenCount && _toTokenId < tokenCount,
            "Invalid token ID"
        );
        require(
            _fromTokenId != 0 && _toTokenId != 0,
            "Use ETH swap functions for ETH"
        );
        ...
        // Check allowance
        require(
            IERC20(fromToken.tokenAddress).allowance(
                msg.sender,
                address(this)
            ) >= _amountToSwap,
        ...
        // Check liquidity
        uint256 toTokenBalance = (_toTokenId == pool.token1Id)
            ? pool.token1Balance
            : pool.token2Balance;
        ...
        // Transfer tokens
        require(
            IERC20(fromToken.tokenAddress).transferFrom(
                msg.sender,
                address(this),
                _amountToSwap
            ),
    ...
    function swapTokensForEth(
        ...
        // Check allowance
        require(
            IERC20(token.tokenAddress).allowance(msg.sender, address(this)) >=
                _amountToSwap,
            "Insufficient allowance"
        );
    ...
    function addLiquidity(
        ...
        require(
            _token1Id < tokenCount && _token2Id < tokenCount,
            "Invalid token ID"
        );
        ...
        if (_token1Id != 0) {
            require(
                IERC20(tokens[_token1Id].tokenAddress).transferFrom(
                    msg.sender,
                    address(this),
                    _token1Amount
                ),
        ...
        if (_token2Id != 0) {
            require(
                IERC20(tokens[_token2Id].tokenAddress).transferFrom(
                    msg.sender,
                    address(this),
                    _token2Amount
                ),
    ...
    function removeLiquidity(
        ...
        if (_token1Id != 0) {
            require(
                IERC20(tokens[_token1Id].tokenAddress).transfer(
                    msg.sender,
                    token1Amount
                ),
                "Token1 transfer failed"
        ...
        if (_token2Id != 0) {
            require(
                IERC20(tokens[_token2Id].tokenAddress).transfer(
                    msg.sender,
                    token2Amount
                ),
                "Token2 transfer failed"
